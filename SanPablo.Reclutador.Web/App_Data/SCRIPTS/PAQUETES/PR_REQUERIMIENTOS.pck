...
CURSOR c_evaluaciones(p_ideCargo CARGO.IDECARGO%TYPE) IS
  SELECT EC.IDEEXAMEVAL, EC.TIPEXAMEN, EC.NOTAMINEXAMEN, EC.TIPAREARESPON, EC.PUNTEXAMEN, EC.INDENTREVFINAL  
...
BEGIN
 ...
 BEGIN
  ...
  FOR REG_EVALUACION IN c_evaluaciones(p_ideCargo) LOOP        
     ...
     (IDEEVALUACIONSOLREQ,IDESOLREQPERSONAL,IDEEXAMEVAL,TIPEXAMEN,NOTAMINEXAMEN,TIPAREARESPON,PUNTEXAMEN,ESTACTIVO,USRCREACION,FECCREACION,INDENTREVFINAL)
     VALUES(IDEEVALUACIONSOLREQ_SQ.NEXTVAL,c_ideSolReqPersonal, REG_EVALUACION.IDEEXAMEVAL, REG_EVALUACION.TIPEXAMEN, REG_EVALUACION.NOTAMINEXAMEN,REG_EVALUACION.TIPAREARESPON,REG_EVALUACION.PUNTEXAMEN,'A',p_usrCreacion,SYSDATE,REG_EVALUACION.INDENTREVFINAL);
  END LOOP;
...
BEGIN
 ...
 BEGIN
  ...
  FOR REG_EVALUACION IN c_evaluaciones(p_ideCargo) LOOP        
     ...
     (IDEEVALUACIONCARGO,IDECARGO,IDEEXAMEVAL,TIPEXAMEN,NOTAMINEXAMEN,TIPAREARESPON,PUNTEXAMEN,ESTACTIVO,USRCREACION,FECCREACION,INDENTREVFINAL)
     VALUES(IDEEVALUACIONCARGO_SQ.NEXTVAL,c_ideCargo_sq, REG_EVALUACION.IDEEXAMEVAL, REG_EVALUACION.TIPEXAMEN, REG_EVALUACION.NOTAMINEXAMEN,REG_EVALUACION.TIPAREARESPON,REG_EVALUACION.PUNTEXAMEN,'A',p_usrCreacion,SYSDATE,REG_EVALUACION.INDENTREVFINAL);
  END LOOP;
    ...
    IF (c_contador = 0) THEN
      ...
      IF(c_tipSolicitud = '01' )THEN   
        FOR REG_EVALUACIONESCARGO IN c_cuEvalCargo(c_ideCargo) LOOP
          ...
          (IDERECLUPERSOEXAMEN,IDERECLUTAPERSONA, IDEEVALUACION, TIPSOLICITUD,TIPESTEVALUACION,IDEEXAMEN,TIPEXAMEN,FECCREACION,USRCREACION,INDENTREVFINAL)   
          VALUES
          (IDERECLUPERSOEXAMEN_SQ.NEXTVAL,p_ideReclutPost,REG_EVALUACIONESCARGO.IDEEVALUACIONCARGO,c_tipSolicitud,'01',REG_EVALUACIONESCARGO.IDEEXAMEVAL,REG_EVALUACIONESCARGO.TIPEXAMEN ,SYSDATE,p_usuarioCreacion,REG_EVALUACIONESCARGO.INDENTREVFINAL);                                          
        END LOOP;
        ELSE
          FOR REG_EVALUACIONES IN c_cuEvalReqSol(c_idSolicitud) LOOP
            ...
            (IDERECLUPERSOEXAMEN,IDERECLUTAPERSONA, IDEEVALUACION, TIPSOLICITUD,TIPESTEVALUACION,IDEEXAMEN,TIPEXAMEN,FECCREACION,USRCREACION,INDENTREVFINAL)   
            VALUES
            (IDERECLUPERSOEXAMEN_SQ.NEXTVAL,p_ideReclutPost,REG_EVALUACIONES.IDEEVALUACIONSOLREQ,c_tipSolicitud,'01',REG_EVALUACIONES.IDEEXAMEVAL, REG_EVALUACIONES.TIPEXAMEN,SYSDATE,p_usuarioCreacion,REG_EVALUACIONES.INDENTREVFINAL);                                          
        END LOOP;  
      END IF;
    END IF;
...
c_codCargo         CARGO.CODCARGO%TYPE;
...
BEGIN
  ...
  FROM (                 
        SELECT PP.IDERECLUTAPERSONA,
               ...
               PP.PROMEDIOEXAMEN PTJESELECCION,
               PP.TIPOAREA,
               PP.RANGOSALARIO,
               ROW_NUMBER()
        ...
        FROM (SELECT RP.IDERECLUTAPERSONA,
             ...
             FROM  POSTULANTE P LEFT JOIN ESTUDIOS_POSTULANTE EP  ON P.IDEPOSTULANTE = EP.IDEPOSTULANTE , 
                   (--Recuperar las filas de recluta persona,las que contienen a postulantes actuales
                     SELECT *
                     FROM (SELECT RP.IDERECLUTAPERSONA,
                                  RP.IDEPOSTULANTE,
                                  RP.IDESOL, 
                                  ...
                                  RP.PROMEDIOEXAMEN,
                                  ROW_NUMBER()
                                  OVER (PARTITION BY rp.idepostulante
                            ORDER BY rp.feccreacion DESC ) AS ROWNUMBER
                            FROM reclutamiento_persona rp
                            WHERE rp.codcargo = c_codCargo
                            AND rp.indpotencial ='S')
                            WHERE ROWNUMBER = 1
                   
                   )RP , 
                   (SELECT *             
                    FROM (SELECT SN.IDECARGO, SN.CODCARGO, SN.IDESEDE IDESEDE,SN.IDEDEPENDENCIA IDEDEPENDENCIA,SN.IDEDEPARTAMENTO IDEDEPARTAMENTO,
                          SN.IDEAREA IDEAREA,SN.NOMBRE NOMBRECARGO,SN.IDESOLNUEVOCARGO IDESOL , '01' TIPSOL 
                          FROM SOLNUEVO_CARGO SN) 
                          UNION ALL
                         (SELECT SR.IDECARGO,SR.CODCARGO, SR.IDESEDE IDESEDE,SR.IDEDEPENDENCIA IDEDEPENDENCIA,SR.IDEDEPARTAMENTO IDEDEPARTAMENTO,
         ...
         WHERE PP.CODCARGO = c_codCargo AND
