...
BEGIN
 ...
 BEGIN
  UPDATE SOLREQ_PERSONAL S SET (S.IDECARGO,S.NOMCARGO,S.DESCARGO, S.SEXO,S.INDSEXO,S.EDADINICIO,S.EDADFIN,S.PUNTEDAD,S.INDEDAD,S.TIPRANGOSALARIO,S.PUNTSALARIO,S.OBJETIVOCARGO,S.FUNCIONESCARGO,S.OBSERVACIONCARGO,
         ...
         S.PUNTTOTHORARIO,S.PUNTTOTUBIGEO,S.PUNTTOTEXAMEN,S.PUNTMINEXAMEN,S.TIPREQUERIMIENTO,S.CANTPRESELEC,S.PUNTMINGRAL)=(
         ...
         C.PUNTTOTHORARIO,C.PUNTTOTUBIGEO,C.PUNTTOTEXAMEN,C.PUNTMINEXAMEN,C.TIPREQUERIMIENTO,C.CANTPRESELEC,C.PUNTMINGRAL
...
c_Consulta1:= 'SELECT SN.IDESOLNUEVOCARGO, PR_INTRANET.FN_ESTADO_SOLICITUD(SN.IDESOLNUEVOCARGO,''N'') ESTADO, '
            ...
            ||'DE.IDEDEPENDENCIA,DE.NOMDEPENDENCIA,DP.IDEDEPARTAMENTO ,DP.NOMDEPARTAMENTO, AR.IDEAREA,AR.NOMAREA, '
    ...
    IF p_nIdDependencia > 0 THEN
      ...
      c_Where := c_Where || '  AND DE.IDEDEPENDENCIA = ' || p_nIdDependencia;
    
    END IF;
  
    IF p_nIdDepartamento > 0 THEN
    
      c_Where := c_Where || '  AND DP.IDEDEPARTAMENTO = ' || p_nIdDepartamento;
    
    END IF;
  
    IF p_nIdArea > 0 THEN
    
      c_Where := c_Where || ' AR.IDEAREA = ' || p_nIdArea;
...
c_Consulta1:= 'SELECT C.ESTACTIVO,C.IDECARGO,C.CODCARGO,C.NOMCARGO,C.DESCARGO,DE.IDEDEPENDENCIA, '
            ||'DE.NOMDEPENDENCIA,DP.IDEDEPARTAMENTO,DP.NOMDEPARTAMENTO, AR.IDEAREA, AR.NOMAREA,C.VERSION ' 
            ||'FROM CARGO C , AREA AR, DEPENDENCIA DE, DEPARTAMENTO DP '
            ||'WHERE C.IDEAREA = AR.IDEAREA '
            ||'AND AR.IDEDEPARTAMENTO = DP.IDEDEPARTAMENTO '
            ||'AND DP.IDEDEPENDENCIA = DE.IDEDEPENDENCIA '
            ||'AND C.IDESEDE = '''||p_cideSede||''' ';
...
CURSOR c_cuEvalCargo(p_ideCargo CARGO.IDECARGO%TYPE) IS
  SELECT EC.IDEEXAMEVAL
  FROM EVALUACION_CARGO EC
  WHERE EC.IDECARGO = c_ideCargo;
    
BEGIN
  
    SELECT RP.IDESOL, RP.TIPSOL , RP.IDECARGO
    INTO c_idSolicitud, c_tipSolicitud,c_ideCargo
    ...
    IF (c_contador = 0) THEN
      ...
      IF(c_tipSolicitud = '01' )THEN   
        FOR REG_EVALUACIONES IN c_cuEvalCargo(c_ideCargo) LOOP
          INSERT INTO RECLU_PERSO_EXAMEN
          (IDERECLUPERSOEXAMEN,IDERECLUTAPERSONA, IDEEVALUACION, TIPSOLICITUD,TIPESTEVALUACION)   
          VALUES
          (IDERECLUPERSOEXAMEN_SQ.NEXTVAL,p_ideReclutPost,REG_EVALUACIONES.IDEEXAMEVAL,c_tipSolicitud,'01');                                          
        END LOOP;
        ELSE
    ...
    BEGIN
      OPEN p_cuExamenes FOR
        SELECT RPE.IDERECLUPERSOEXAMEN,RPE.IDERECLUTAPERSONA, RPE.IDEEVALUACION,
         (DECODE(RPE.TIPSOLICITUD,'01',(SELECT EC.IDEEXAMEVAL FROM EVALUACION_CARGO EC WHERE EC.IDEEVALUACIONCARGO = IDEEVALUACION),(SELECT ES.IDEEXAMEVAL FROM EVALUACION_SOLREQ ES WHERE ES.IDEEVALUACIONSOLREQ = IDEEVALUACION))) IDEEXXAMENVAL,
         EX.NOMEXAMEN,EX.TIPEXAMEN,PR_INTRANET.FN_VALOR_GENERAL('TIPOCRITERIO',EX.TIPEXAMEN) TIPOEXAMEN,
         RPE.IDROLRESPONSABLE, R.DSCROL,RPE.TIPESTEVALUACION, PR_INTRANET.FN_VALOR_GENERAL('ESTADOEVALUACIO',RPE.TIPESTEVALUACION) ESTADOEVALUACION,
         RPE.FECEVALUACION,RPE.HORAEVALUACION, RPE.NOTAFINAL,RPE.COMENTARIORESUL  
        FROM RECLU_PERSO_EXAMEN RPE, ROL R , (SELECT * FROM EXAMEN) EX
        WHERE RPE.IDROLRESPONSABLE = R.IDROL
        AND RPE.IDERECLUTAPERSONA = p_ideReclutPost;
