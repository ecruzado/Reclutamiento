create or replace package PR_INTRANET_ED is
  ...
  PROCEDURE SP_INSERTA_RECLUTA_EXAMEM(p_nIDERECLUTAPERSONA     IN RECLU_PERSO_EXAMEN.IDERECLUTAPERSONA%TYPE,
                                      ...
                                      p_cTIPESTEVALUACION      IN RECLU_PERSO_EXAMEN.TIPESTEVALUACION%TYPE,
                                      p_cOBSERVACION           IN RECLU_PERSO_EXAMEN.OBSERVACION%TYPE,
...
create or replace package body PR_INTRANET_ED is
      ...
      OPEN p_cRetVal FOR
        ...
        SELECT Y.TIPPUESTO,
          ...
          FROM (SELECT X.TIPPUESTO,
                 ...
                 GROUP BY X.TIPPUESTO, X.IDECARGO, X.IDESEDE, X.NOMCARGO
                 ) Y
         WHERE (nvl(p_cFechaInicio, null) IS NULL OR
               (Y.FECINICIALMAX >= TO_DATE(p_cFechaInicio, 'DD/MM/YYYY')))
           AND (nvl(p_cFecFin, null) IS NULL OR
               (Y.FECFINALMAX < TO_DATE(p_cFecFin, 'DD/MM/YYYY') + 1))
        --ORDER BY Y.FECINICIALMAX DESC,Y.FECINICIALMAX DESC



        ) Z ORDER BY FECINICIALMAX DESC,FECFINALMAX ASC;
  ...
  PROCEDURE SP_POSTULACION(p_nIdPostulante IN NUMBER,
    ...
    CURSOR C_CONOCIMIENTOS_POSTULANTE IS
      SELECT TIPCONOFIMATICA,
             ...
             INDCERTIFICACION,
             (CASE WHEN TIPNIVELCONOCIMIENTO = P_NIVELBASICO THEN 1
    ...
    FROM (
      SELECT TIPCARGOTRABAJO, FECTRABINICIO, FECTRABFIN, INDTRABACTUALMENTE,
            (CASE WHEN 'S' = INDTRABACTUALMENTE then months_between(sysdate, FECTRABINICIO)
                  ELSE  months_between(FECTRABFIN,FECTRABINICIO) END) TIEMPOEXP 
  ...
  BEGIN
    ...
    BEGIN
        ...
        IF C1.TIPSOL = '01' THEN
          ...
          FOR C4 IN C_CONOCIMIENTOS_POSTULANTE LOOP
            FOR C5 IN C_CONOCIMIENTOS_CARGO LOOP
              ...
              IF C4.TIPCONOFIMATICA = C5.TIPCONOFIMATICA AND C4.TIPNOMOFIMATICA = C5.TIPNOMOFIMATICA THEN
                IF C4.NIVELCONO >= C5.NIVELCONO THEN
                   nCont := nCont + C5.PUNTCONOCIMIENTO;
                END IF;
              END IF;

              IF C4.TIPIDIOMA = C5.TIPIDIOMA AND C4.TIPCONOCIDIOMA = C5.TIPCONOCIDIOMA THEN
                IF C4.NIVELCONO >= C5.NIVELCONO THEN
                   nCont := nCont + C5.PUNTCONOCIMIENTO;
                END IF;
              END IF;

              IF C4.TIPCONOCGENERALES = C5.TIPCONOGENERAL  AND C4.TIPNOMCONOCGRALES = C5.TIPNOMCONOCGRALES  THEN
                IF C4.NIVELCONO >= C5.NIVELCONO THEN
                  nCont := nCont + C5.PUNTCONOCIMIENTO;
                END IF;
              END IF;
      ...
      IF nTotal >= nPuntMin THEN
        ...
        IF nCantPreseleSol > nCantPreseleMin THEN --jccana
  ...
  PROCEDURE SP_OBTIENE_POTENCIALES(p_nIdSol     IN NUMBER,
    ...
    nNumExamenes             NUMBER;
    nNumMeses                NUMBER;
    nNumMesesConf            NUMBER;
    nVersionActual           NUMBER;
    nVersionAnterior         NUMBER;
    nContSol                 NUMBER;
  ...
  BEGIN
    ...
    OPEN p_Rpta FOR
      SELECT P.TIPDOCUMENTO CODTIPDOCUMENTO,
             ...
             decode(to_char(P.fecmodificacion,'DD/MM/YYYY'),'01/01/0001',(upper(DECODE(P.Feccreacion,
                    NULL,
                    '',
             ...
             (upper(DECODE(P.fecmodificacion,
                    NULL,
                    '',
                    TO_CHAR(P.fecmodificacion, 'DD')) || ' de ' ||
             PR_INTRANET_ED.FN_DES_MES(DECODE(P.fecmodificacion,
                                              NULL,
                                              0,
                                              TO_CHAR(P.fecmodificacion, 'MM')),
                                       1) || ' de ' ||
  ...
  BEGIN
    ...
    OPEN p_Rpta FOR
      SELECT E.IDEESTUDIOSPOSTULANTE,
             (decode(E.TIPNOMINSTITUCION,'XX',E.NOMINSTITUCION,(SELECT DG.DESCRIPCION
                FROM DETALLE_GENERAL DG
               WHERE DG.IDEGENERAL = 17
                 AND DG.VALOR = E.TIPNOMINSTITUCION))
                 ) INSTITUCION,
...
BEGIN
  ...
  OPEN P_CRETVAL FOR
    SELECT (SELECT U.EMAIL FROM USUARIO U WHERE U.IDUSUARIO = E.IDUSUARIO) EMAIL
      FROM USUAROLSEDE E
